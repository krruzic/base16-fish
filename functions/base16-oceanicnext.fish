# base16-fish (https://github.com/tomyun/base16-fish)
# based on base16-shell (https://github.com/chriskempson/base16-shell)
# OceanicNext scheme by https://github.com/voronianski/oceanic-next-color-scheme

function base16-oceanicnext -d "OceanicNext"
  set -gx color00 '#1B2B34' # Base 00 - Black
  set -gx color01 '#EC5f67' # Base 08 - Red
  set -gx color02 '#99C794' # Base 0B - Green
  set -gx color03 '#FAC863' # Base 0A - Yellow
  set -gx color04 '#6699CC' # Base 0D - Blue
  set -gx color05 '#C594C5' # Base 0E - Magenta
  set -gx color06 '#5FB3B3' # Base 0C - Cyan
  set -gx color07 '#C0C5CE' # Base 05 - White
  set -gx color08 '#65737E' # Base 03 - Bright Black
  set -gx color09 $color01 # Base 08 - Bright Red
  set -gx color10 $color02 # Base 0B - Bright Green
  set -gx color11 $color03 # Base 0A - Bright Yellow
  set -gx color12 $color04 # Base 0D - Bright Blue
  set -gx color13 $color05 # Base 0E - Bright Magenta
  set -gx color14 $color06 # Base 0C - Bright Cyan
  set -gx color15 '#D8DEE9' # Base 07 - Bright White
  set -gx color16 '#F99157' # Base 09
  set -gx color17 '#AB7967' # Base 0F
  set -gx color18 '#343D46' # Base 01
  set -gx color19 '#4F5B66' # Base 02
  set -gx color20 '#A7ADBA' # Base 04
  set -gx color21 '#CDD3DE' # Base 06
  set -gx colorfg $color07 # Base 05 - White
  set -gx colorbg $color00 # Base 00 - Black
  __base16_set_colors
  if type -q fzf
    __base16_set_fzf
  end
end
